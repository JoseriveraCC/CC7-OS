...
void timer_init(void) {
    ...
    os_write("Timer listo...\n");

    // Desenmascarar IRQ 68 en el controlador de interrupciones
    PUT32(INTC_MIR_CLEAR2, 0x10); 
    os_write("Controlador listo...\n");
    
    // Configurar prioridad de interrupción
    PUT32(INTC_ILR68, 0x0);
    os_write("Prioridad de interrupcion listo...\n");
    ...
    // Establecer el valor de carga para 2 segundos
    PUT32(TLDR, 0xFE91CA00);

    // Establecer el contador al mismo valor
    PUT32(TCRR, 0xFE91CA00);

    // Habilitar interrupción por overflow
    PUT32(TIER, 0x2);

    // Iniciar timer en modo auto-recarga
    PUT32(TCLR, 0x3);
...
int main(void) {
    ...
    timer_init();
    os_write("Timer configurado a 2 segundos.\n");
    enable_irq();
    os_write("Interrupciones habilitadas. Entrando al bucle principal...\n");
